...
Each method's scoring is stored in a list so it must first be extracted before further analysis can be done. The method's are stored in the list in this order: afinn, nrc, then bing. The list must be indexed to access each of these newspaper's specific method's dataframes from earlier.
...
```{r echo=FALSE, include = FALSE}
afinn<- data.frame(row.names = c("Newspaper", "Average Value"))

avg_afinn<-function(afinn_data){}

head(afinn)
```

### NRC Sentiment Analysis Conclusions
To evaluate the nrc method on these six newspapers, we further consolidated using the sentiment associated with each word so that the first column contained the newspaper, the second column contained the sentiments, and the third column contained the total count for the newspaper.
```{r echo=FALSE, include = FALSE}
nrc<- data.frame(row.names = c("Newspaper", "Sentiment", "Total Count"))

avg_nrc<-function(nrc_data){}
