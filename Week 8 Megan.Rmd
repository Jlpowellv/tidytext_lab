...
Our team will be evaluating documents from 2010-present on artifical intelligence. Megan will be researching newspapers from Asia-- specifically China Daily and the Korea Herald.
...
>>>>>>> 6b6fd2a3ca9ed8786398bf270f09b9f453ee4b83
...
Read in data from each source (China Daily and the Korea Hearld)
```{r echo=FALSE}
...
Iterate through each file and extract the data using an rtf reader & turn it into a tibble
Rename the column of data to have the name "article" for easier access
Get the word count in the article
Use sentiment analysis on the article

tf-idf
```{r echo=FALSE, include = FALSE}

china_afinn<-data.frame(row.names=c("word", "name", "value"))
china_nrc<-data.frame(row.names=c("word", "n", "sentiment"))
china_bing<-data.frame(row.names=c("word", "n", "sentiment"))
...
for (file in china_files){
  ...
  china_afinn <- rbind(china_afinn, data_afinn)
  
  data_nrc <- data %>%
    inner_join(get_sentiments("nrc"))
  china_nrc <- rbind(china_nrc, data_nrc)
  
  data_bing <- data %>%
    inner_join(get_sentiments("bing"))
  
  china_bing <- rbind(china_bing, data_bing)
}

```

Aggregate the data from each article to get the total count of words
```{r echo=FALSE, include = FALSE}
agg_china_afinn <- china_afinn %>% group_by(word, value) %>% summarise(TotalCount = sum(n))
agg_china_nrc <- china_nrc %>% group_by(word, sentiment) %>% summarise(TotalCount = sum(n))
agg_china_bing <- china_bing %>% group_by(word, sentiment) %>% summarise(TotalCount = sum(n))
```